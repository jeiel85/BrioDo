export const translations = {
  ko: {
    reminders: 'BrioDo',
    incomplete: '개의 미완료 항목',
    doneAll: '일정을 모두 마쳤습니다!',
    completed: '완료됨',
    noTime: '시간 지정 없음',
    placeholder: '무엇을 도와드릴까요?',
    date: '날짜',
    time: '시간',
    tags: '태그 (#으로 여러 개 입력)',
    cancel: '취소',
    save: '저장',
    delete: '삭제',
    aiThinking: 'AI가 분석 중...',
    viewDaily: '날짜별',
    viewAll: '전체',
    allTags: '모든 태그',
    today: '오늘',
    langName: '한국어',
    login: '로그인',
    logout: '로그아웃',
    loading: '연결 중...',
    search: '검색',
    searchPlaceholder: '제목, 태그, 설명 검색...',
    calendarExpired: '캘린더 연결이 만료됐습니다. 탭하여 재연결하세요.',
    repeat: '반복',
    repeatNone: '없음',
    repeatDaily: '매일',
    repeatWeekly: '매주',
    repeatMonthly: '매월',
    repeatEndDate: '종료일',
    checklist: '체크리스트',
    checklistPlaceholder: '세부 항목...',
    pending: '인박스',
    lists: '컬렉션',
    progress: '통계',
    allTasks: '전체 할 일',
    uncategorized: '미분류',
    weeklyActivity: '주간 활동',
    todayDone: '오늘 완료',
    weekDone: '이번 주',
    allDone: '전체 완료',
    achievements: '업적',
    streakLabel: '일 연속',
    inboxZero: '인박스 제로',
    focusBadge: '집중 모드',
    recentDone: '최근 완료',
    noRecentDone: '아직 완료된 항목이 없어요',
    tasksLeft: '개 남음',
    tasksTotal: '개',
    completionRate: '완료율',
    viewTasks: '할 일 보기',
    aiBriefing: 'AI 브리핑',
    morningBriefing: '아침 브리핑',
    eveningBriefing: '저녁 브리핑',
    briefingTime: '브리핑 시간',
    briefingEnabled: 'AI 브리핑 활성화',
    briefingLoading: '브리핑 생성 중...',
    briefingMorningTitle: '오늘의 일정',
    briefingEveningTitle: '오늘의 리뷰',
    noBriefingData: '오늘 일정이 없습니다',
    briefingMorningDefault: '08:00',
    briefingEveningDefault: '21:00',
  },
  en: {
    reminders: 'BrioDo',
    incomplete: ' incomplete',
    doneAll: 'Everything done!',
    completed: 'Completed',
    noTime: 'No time',
    placeholder: 'What do you want to remind?',
    date: 'Date',
    time: 'Time',
    tags: 'Tags (use # for multiple)',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    aiThinking: 'AI refining...',
    viewDaily: 'Daily',
    viewAll: 'All',
    allTags: 'All Tags',
    today: 'Today',
    langName: 'English',
    login: 'Login',
    logout: 'Logout',
    loading: 'Connecting...',
    search: 'Search',
    searchPlaceholder: 'Search title, tags, description...',
    calendarExpired: 'Calendar connection expired. Tap to reconnect.',
    repeat: 'Repeat',
    repeatNone: 'None',
    repeatDaily: 'Daily',
    repeatWeekly: 'Weekly',
    repeatMonthly: 'Monthly',
    repeatEndDate: 'End date',
    checklist: 'Checklist',
    checklistPlaceholder: 'Sub-item...',
    pending: 'Inbox',
    lists: 'Collections',
    progress: 'Stats',
    allTasks: 'All Tasks',
    uncategorized: 'Uncategorized',
    weeklyActivity: 'Weekly Activity',
    todayDone: 'Today Done',
    weekDone: 'This Week',
    allDone: 'All Time',
    achievements: 'Achievements',
    streakLabel: '-day streak',
    inboxZero: 'Inbox Zero',
    focusBadge: 'Focus Mode',
    recentDone: 'Recently Done',
    noRecentDone: 'No completed tasks yet',
    tasksLeft: ' left',
    tasksTotal: ' tasks',
    completionRate: 'Done',
    viewTasks: 'View Tasks',
    aiBriefing: 'AI Briefing',
    morningBriefing: 'Morning Briefing',
    eveningBriefing: 'Evening Briefing',
    briefingTime: 'Briefing Time',
    briefingEnabled: 'Enable AI Briefing',
    briefingLoading: 'Generating briefing...',
    briefingMorningTitle: "Today's Schedule",
    briefingEveningTitle: "Today's Review",
    noBriefingData: 'No tasks for today',
    briefingMorningDefault: '08:00',
    briefingEveningDefault: '21:00',
  },
  ja: {
    reminders: 'BrioDo',
    incomplete: ' 개의 미완료',
    doneAll: '予定はすべて完了しました!',
    completed: '完了済み',
    noTime: '時間指定なし',
    placeholder: '何を思い出させますか？',
    date: '日付',
    time: '時間',
    tags: 'タグ（#を使用して複数）',
    cancel: 'キャンセル',
    save: '保存',
    delete: '削除',
    aiThinking: 'AIが整理しています...',
    viewDaily: '日別',
    viewAll: 'すべて',
    allTags: 'すべてのタグ',
    today: '今日',
    langName: '日本語',
    login: 'ログイン',
    logout: 'ログアウト',
    loading: '接続中...',
    search: '検索',
    searchPlaceholder: 'タイトル、タグ、説明を検索...',
    calendarExpired: 'カレンダー接続が切れました。タップして再接続。',
    repeat: '繰り返し',
    repeatNone: 'なし',
    repeatDaily: '毎日',
    repeatWeekly: '毎週',
    repeatMonthly: '毎月',
    repeatEndDate: '終了日',
    checklist: 'チェックリスト',
    checklistPlaceholder: 'サブ項目...',
    pending: 'インボックス',
    lists: 'コレクション',
    progress: '統計',
    allTasks: 'すべてのタスク',
    uncategorized: '未分類',
    weeklyActivity: '週間活動',
    todayDone: '今日完了',
    weekDone: '今週',
    allDone: '全完了',
    achievements: '実績',
    streakLabel: '日連続',
    inboxZero: 'インボックスゼロ',
    focusBadge: 'フォーカスモード',
    recentDone: '最近完了',
    noRecentDone: 'まだ完了したタスクがありません',
    tasksLeft: ' 残り',
    tasksTotal: ' 件',
    completionRate: '完了',
    viewTasks: 'タスクを見る',
    aiBriefing: 'AIブリーフィング',
    morningBriefing: '朝のブリーフィング',
    eveningBriefing: '夜のブリーフィング',
    briefingTime: 'ブリーフィング時間',
    briefingEnabled: 'AIブリーフィングを有効化',
    briefingLoading: 'ブリーフィング生成中...',
    briefingMorningTitle: '今日の予定',
    briefingEveningTitle: '今日のレビュー',
    noBriefingData: '今日のタスクはありません',
    briefingMorningDefault: '08:00',
    briefingEveningDefault: '21:00',
  },
  zh: {
    reminders: 'BrioDo',
    incomplete: ' 个待办事项',
    doneAll: '完成！',
    completed: '已完成',
    noTime: '未设置时间',
    placeholder: '您想提醒什么？',
    date: '日期',
    time: '时间',
    tags: '标签（使用 #）',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    aiThinking: 'AI正在整理...',
    viewDaily: '按日',
    viewAll: '全部',
    allTags: '所有标签',
    today: '今天',
    langName: '中文',
    login: '登录',
    logout: '登出',
    loading: '同步中...',
    search: '搜索',
    searchPlaceholder: '搜索标题、标签、描述...',
    calendarExpired: '日历连接已过期，点击重新连接。',
    repeat: '重复',
    repeatNone: '无',
    repeatDaily: '每天',
    repeatWeekly: '每周',
    repeatMonthly: '每月',
    repeatEndDate: '结束日',
    checklist: '清单',
    checklistPlaceholder: '子项目...',
    pending: '收件箱',
    lists: '收藏',
    progress: '统计',
    allTasks: '全部任务',
    uncategorized: '未分类',
    weeklyActivity: '本周活动',
    todayDone: '今日完成',
    weekDone: '本周',
    allDone: '全部完成',
    achievements: '成就',
    streakLabel: '天连续',
    inboxZero: '清空收件箱',
    focusBadge: '专注模式',
    recentDone: '最近完成',
    noRecentDone: '还没有完成的任务',
    tasksLeft: ' 剩余',
    tasksTotal: ' 件',
    completionRate: '完成',
    viewTasks: '查看任务',
    aiBriefing: 'AI简报',
    morningBriefing: '早间简报',
    eveningBriefing: '晚间简报',
    briefingTime: '简报时间',
    briefingEnabled: '启用AI简报',
    briefingLoading: '正在生成简报...',
    briefingMorningTitle: '今日日程',
    briefingEveningTitle: '今日回顾',
    noBriefingData: '今天没有任务',
    briefingMorningDefault: '08:00',
    briefingEveningDefault: '21:00',
  }
}
