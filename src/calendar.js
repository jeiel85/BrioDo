...
export const createBrioDoCalendar = async (token) => {
  const res = await fetch('https://www.googleapis.com/calendar/v3/calendars', {
    ...
    body: JSON.stringify({ summary: 'BrioDo', timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone })
  })
...
export const ensureBrioDoCalendar = async () => {
  ...
  _ensureCalendarPromise = (async () => {
    try {
      ...
      console.error('ensureBrioDoCalendar error:', error)
...
export const syncEventToGoogle = async (todo) => {
  ...
  const calendarId = await ensureBrioDoCalendar()
