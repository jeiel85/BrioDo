...
export const ACHIEVEMENT_DEFS = [
  ...
  { id: 'X1', icon: '🌋', hidden: true, difficulty: 10, category: 'streak', name: n('200일의 용암','Lava Streak','200日の溶岩','200日熔岩'), desc: n('200일 연속 최소 1개 완료','200-day streak','200日連続完了','连续200天完成'), check: s => s.streak >= 200 },
  { id: 'X2', icon: '🌞', hidden: true, difficulty: 10, category: 'streak', name: n('1년의 기적','Year Miracle','1年の奇跡','一年奇迹'), desc: n('365일 연속 최소 1개 완료','365-day streak','365日連続完了','连续365天完成'), check: s => s.streak >= 365 },
  { id: 'X3', icon: '🌅', hidden: true, difficulty: 9, category: 'daily', name: n('새벽의 전사','Dawn Warrior','夜明けの戦士','黎明战士'), desc: n('새벽 5시 전 완료 30번 누적','30 completions before 5am','午前5時前に30回完了','凌晨5点前累计完成30次'), check: s => (s.flags?.dawn5am || 0) >= 30 },
  { id: 'X4', icon: '🏔️', hidden: true, difficulty: 10, category: 'completion', name: n('5000의 산','5000 Peak','5000の山','5000之山'), desc: n('누적 5000개 완료','5000 total completions','累計5000件完了','累计完成5000件'), check: s => s.totalCompleted >= 5000 },
  { id: 'X5', icon: '🌌', hidden: true, difficulty: 10, category: 'completion', name: n('만 개의 별','10K Stars','1万の星','万颗星'), desc: n('누적 10000개 완료','10000 total completions','累計10000件完了','累计完成10000件'), check: s => s.totalCompleted >= 10000 },
  { id: 'X6', icon: '🌪️', hidden: true, difficulty: 10, category: 'daily', name: n('하루 50개','Daily 50','1日50件','每日50件'), desc: n('하루에 50개 이상 완료','Complete 50+ tasks in one day','1日に50件以上完了','一天完成50件以上'), check: s => s.todayCompleted >= 50 },
  { id: 'X7', icon: '🎄', hidden: true, difficulty: 5, category: 'special', name: n('크리스마스의 기적','Christmas Miracle','クリスマスの奇跡','圣诞奇迹'), desc: n('12월 25일에 할 일 완료 (비밀 보상)','Complete a task on December 25th','12月25日にタスクを完了','在12月25日完成任务'), check: s => s.special?.christmas },
  { id: 'X8', icon: '🎆', hidden: true, difficulty: 5, category: 'special', name: n('새해 첫 할 일','New Year First','新年初タスク','新年第一件'), desc: n('1월 1일에 할 일 완료 (비밀 보상)','Complete a task on January 1st','1月1日にタスクを完了','在1月1日完成任务'), check: s => s.special?.newYear },
  { id: 'X9', icon: '💘', hidden: true, difficulty: 5, category: 'special', name: n('발렌타인 계획자','Valentine Planner','バレンタイン計画者','情人节规划者'), desc: n('2월 14일에 할 일 완료 (비밀 보상)','Complete a task on February 14th','2月14日にタスクを完了','在2月14日完成任务'), check: s => s.special?.valentine },
  { id: 'X10', icon: '🎃', hidden: true, difficulty: 5, category: 'special', name: n('할로윈 완료','Halloween Done','ハロウィン完了','万圣节完成'), desc: n('10월 31일에 할 일 완료 (비밀 보상)','Complete a task on October 31st','10月31日にタスクを完了','在10月31日完成任务'), check: s => s.special?.halloween },
  { id: 'X11', icon: '⚡', hidden: true, difficulty: 9, category: 'daily', name: n('번개 완료','Lightning Done','雷の完了','闪电完成'), desc: n('등록 후 5분 내 완료 10회','10 tasks completed within 5 minutes of adding','追加後5分以内に10回完了','添加后5分钟内完成10次'), check: s => (s.flags?.lightning5m || 0) >= 10 },
  { id: 'X12', icon: '🎯', hidden: true, difficulty: 10, category: 'daily', name: n('저격수','Sniper','狙撃手','狙击手'), desc: n('등록 후 1분 내 완료 5회','5 tasks completed within 1 minute of adding','追加後1分以内に5回完了','添加后1分钟内完成5次'), check: s => (s.flags?.sniper1m || 0) >= 5 },
  { id: 'X13', icon: '🌙', hidden: true, difficulty: 8, category: 'special', name: n('자정의 전사','Midnight Warrior','深夜の戦士','午夜战士'), desc: n('00:00~00:05 사이에 할 일 완료 5회','5 tasks completed between 00:00-00:05','00:00〜00:05の間に5回完了','在00:00-00:05之间完成5次'), check: s => (s.flags?.midnight5min || 0) >= 5 },
  { id: 'X14', icon: '🎨', hidden: true, difficulty: 8, category: 'tag', name: n('태그 마스터 X','Tag Master X','タグマスターX','标签大师X'), desc: n('서로 다른 태그 20가지 사용 (비밀 보상)','Use 20 different tags (secret bonus)','20種類のタグを使用','使用20种不同标签'), check: s => s.uniqueTagCount >= 20 },
  { id: 'X15', icon: '👑', hidden: true, difficulty: 10, category: 'priority', name: n('우선순위 황제','Priority Emperor','優先度皇帝','优先级皇帝'), desc: n('4가지 우선순위 각 50개씩 완료','Complete 50 tasks at each priority level','各優先度50件完了','每种优先级完成50件'), check: s => s.completedByPriority.low >= 50 && s.completedByPriority.medium >= 50 && s.completedByPriority.high >= 50 && s.completedByPriority.urgent >= 50 },
  { id: 'X16', icon: '📅', hidden: true, difficulty: 7, category: 'weekly', name: n('모든 요일 정복','All Weekdays','全曜日征服','征服所有工作日'), desc: n('월~일 모든 요일에 최소 1개씩 완료','Complete at least 1 task on every day of the week','月〜日全曜日に1件以上完了','一周每天都完成至少1件'), check: s => (s.flags?.allWeekdays || 0) >= 7 },
  { id: 'X17', icon: '🕐', hidden: true, difficulty: 10, category: 'daily', name: n('24시간 전사','24-Hour Warrior','24時間戦士','24小时战士'), desc: n('하루 0~23시 모든 시간대에 완료 기록','Complete tasks in all 24 hours of the day','0〜23時の全時間帯に完了記録','在一天的所有24个小时都完成任务'), check: s => (s.flags?.allHours || 0) >= 24 },
  { id: 'X18', icon: '🔔', hidden: true, difficulty: 7, category: 'special', name: n('정각 챌린저','On-The-Hour','正刻チャレンジャー','整点挑战者'), desc: n('정각(분=0)에 할 일 완료 7회','Complete 7 tasks exactly on the hour','正時(分=0)に7回完了','在整点完成7次任务'), check: s => (s.flags?.onTheHour || 0) >= 7 },
  { id: 'X19', icon: '🤖', hidden: true, difficulty: 9, category: 'ai', name: n('AI 백 개 X','AI 100 X','AI百件X','AI100件X'), desc: n('AI 입력으로 100개 할 일 생성 (비밀 보상)','Create 100 tasks using AI (secret bonus)','AI入力で100件タスク作成','用AI输入创建100件任务'), check: s => (s.flags?.aiTasks || 0) >= 100 },
  { id: 'X20', icon: '🎙️', hidden: true, difficulty: 8, category: 'ai', name: n('음성 오십 X','Voice 50 X','音声50件X','语音50件X'), desc: n('음성 입력으로 50개 할 일 생성 (비밀 보상)','Create 50 tasks using voice (secret bonus)','音声入力で50件タスク作成','用语音输入创建50件任务'), check: s => (s.flags?.voiceTasks || 0) >= 50 },
  { id: 'X21', icon: '🧬', hidden: true, difficulty: 8, category: 'ai', name: n('AI+음성 콤보','AI+Voice Combo','AI+音声コンボ','AI+语音组合'), desc: n('하루에 AI와 음성 각각 5회 이상 사용','Use both AI and voice 5+ times each in one day','1日にAIと音声それぞれ5回以上使用','一天内AI和语音各使用5次以上'), check: s => s.flags?.aiAndVoiceDay },
  { id: 'X22', icon: '📅', hidden: true, difficulty: 9, category: 'calendar', name: n('캘린더 백 개 X','Calendar 100 X','カレンダー100件X','日历100件X'), desc: n('Google 캘린더 이벤트 100개 동기화 (비밀 보상)','100 tasks synced with calendar (secret bonus)','カレンダー連携100件','日历同步100件'), check: s => s.calSyncCount >= 100 },
  { id: 'X23', icon: '🔗', hidden: true, difficulty: 8, category: 'calendar', name: n('캘린더 한 달','Calendar Month','カレンダー1ヶ月','日历一个月'), desc: n('30일 동안 캘린더 동기화 상태 유지','Keep calendar sync active for 30 days','30日間カレンダー連携を維持','保持日历同步30天'), check: s => s.flags?.calendarMonth30 },
  { id: 'X24', icon: '📝', hidden: true, difficulty: 10, category: 'subtask', name: n('하위 태스크 500','500 Subtasks','サブタスク500件','500个子任务'), desc: n('하위 태스크 누적 500개 완료','500 total subtask completions','サブタスク累計500件完了','累计完成500个子任务'), check: s => s.totalSubtasksCompleted >= 500 },
  { id: 'X25', icon: '🧩', hidden: true, difficulty: 10, category: 'subtask', name: n('초대형 계획','Mega Plan','超大型計画','超大型计划'), desc: n('하위 태스크 20개 이상인 할 일 완료','Complete a task with 20+ subtasks','サブタスク20件以上のタスクを完了','完成含20个以上子任务的任务'), check: s => s.flags?.bigPlan20 },
  { id: 'X26', icon: '📦', hidden: true, difficulty: 9, category: 'completion', name: n('할 일 천 개 등록','1000 Registered','タスク千件登録','登记1000件'), desc: n('총 등록 할 일 수 1000개 달성','Register 1000 total tasks (completed or not)','合計1000件タスクを登録','累计登记1000件任务'), check: s => s.totalRegistered >= 1000 },
  { id: 'X27', icon: '🔥', hidden: true, difficulty: 10, category: 'weekly', name: n('월간 500','Month 500','月間500件','月度500件'), desc: n('한 달에 500개 이상 완료','500+ completions in a month','1ヶ月に500件以上完了','一个月内完成500件以上'), check: s => s.monthCompleted >= 500 },
  { id: 'X28', icon: '💥', hidden: true, difficulty: 9, category: 'daily', name: n('24시간 20개','24h Burst','24時間20件','24小时20件'), desc: n('24시간 안에 20개 완료','Complete 20 tasks within 24 hours','24時間以内に20件完了','24小时内完成20件'), check: s => s.flags?.burst24h20 },
  { id: 'X29', icon: '🌟', hidden: true, difficulty: 10, category: 'streak', name: n('100일 × 평균 5개','100 Days × 5 Avg','100日×5件平均','100天×平均5件'), desc: n('100일 연속 + 하루 평균 5개 이상','100-day streak AND 5+ tasks/day average','100日連続＋1日平均5件以上','连续100天且每天平均5件以上'), check: s => s.streak >= 100 && s.flags?.avg5perDay100 },
  { id: 'X30', icon: '☀️', hidden: true, difficulty: 10, category: 'daily', name: n('50일 얼리버드','50-Day Early Bird','50日アーリーバード','50天早起鸟'), desc: n('50일 연속 오전 9시 전 완료','50 days in a row completing tasks before 9am','50日連続午前9時前完了','连续50天在上午9点前完成任务'), check: s => s.flags?.earlyBirdStreak50 },
  { id: 'X31', icon: '💯', hidden: true, difficulty: 10, category: 'weekly', name: n('완벽한 한 달','Perfect Month','完璧な1ヶ月','完美一个月'), desc: n('한 달 동안 등록한 모든 할 일 완료','Complete every task registered in a month','1ヶ月間の全タスクを完了','完成一个月内登记的所有任务'), check: s => s.flags?.perfectMonth },
  { id: 'X32', icon: '🏆', hidden: true, difficulty: 10, category: 'weekly', name: n('3달 연속 100+','3 Months 100+','3ヶ月連続100+','连续3月100+'), desc: n('3개월 연속 월별 100개 이상 완료','100+ completions for 3 consecutive months','3ヶ月連続で月100件以上完了','连续3个月每月完成100件以上'), check: s => s.flags?.month100x3 },
  { id: 'X33', icon: '🦉', hidden: true, difficulty: 8, category: 'daily', name: n('올빼미 100','Night Owl 100','夜型100','夜猫子100'), desc: n('밤 11시 이후 할 일 100개 완료','Complete 100+ tasks after 11pm','夜11時以降100件完了','晚上11点后完成100件'), check: s => s.nightOwl >= 100 },
  { id: 'X34', icon: '🌃', hidden: true, difficulty: 9, category: 'daily', name: n('30일 밤샘','30 Night Streak','30日夜更かし','30天夜间连续'), desc: n('30일 연속 밤 11시 이후 완료','Complete tasks after 11pm for 30 days in a row','30日連続夜11時以降完了','连续30天在晚上11点后完成任务'), check: s => s.flags?.nightStreak30 },
  { id: 'X35', icon: '🎖️', hidden: true, difficulty: 10, category: 'priority', name: n('우선순위 전설','Priority Legend','優先度の伝説','优先级传奇'), desc: n('모든 우선순위 각 100개씩 완료','Complete 100 tasks at each priority level','各優先度100件完了','每种优先级完成100件'), check: s => s.completedByPriority.low >= 100 && s.completedByPriority.medium >= 100 && s.completedByPriority.high >= 100 && s.completedByPriority.urgent >= 100 },
  { id: 'X36', icon: '🚨', hidden: true, difficulty: 9, category: 'priority', name: n('긴급 하루 10','Urgent Day 10','緊急1日10件','紧急一天10件'), desc: n('하루에 긴급 할 일 10개 완료','Complete 10 urgent tasks in one day','1日に緊急タスク10件完了','一天完成10件紧急任务'), check: s => s.flags?.urgentDay10 },
  { id: 'X37', icon: '🔄', hidden: true, difficulty: 10, category: 'recurrence', name: n('반복 100회','100 Reps X','繰り返し100回X','100次重复X'), desc: n('같은 반복 일정 100회 완료','Complete the same recurring task 100 times','同じ繰り返しタスクを100回完了','同一重复任务完成100次'), check: s => s.flags?.sameRecurring100 },
  { id: 'X38', icon: '🌀', hidden: true, difficulty: 10, category: 'recurrence', name: n('반복 오중주','Recurring Quintet','繰り返し五重奏','重复五重奏'), desc: n('5가지 다른 반복 일정 각 30회씩 완료','Complete 5 different recurring tasks 30 times each','5種の繰り返しタスクをそれぞれ30回完了','5种不同重复任务各完成30次'), check: s => s.flags?.multiRecurring5x30 },
  { id: 'X39', icon: '📆', hidden: true, difficulty: 10, category: 'engagement', name: n('365일 사용','365 Days Used','365日使用','使用365天'), desc: n('앱 사용 총 365일 (비연속 허용)','Use the app for 365 total days','アプリを累計365日使用','累计使用应用365天'), check: s => (s.flags?.totalOpens || 0) >= 365 },
  { id: 'X40', icon: '📱', hidden: true, difficulty: 9, category: 'engagement', name: n('천 번 실행','1000 Launches','千回起動','1000次启动'), desc: n('앱 실행 1000회','Launch the app 1000 times','アプリを1000回起動','应用启动1000次'), check: s => (s.flags?.totalOpens || 0) >= 1000 },
