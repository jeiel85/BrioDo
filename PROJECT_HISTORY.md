...
- **2026-03-22** (세션 7):
  ...
  - **자연어 처리 불동작** — 로그인 실패의 2차 문제로 확인. `handleSmartSave`는 로그인 상태에서만 AI 분석을 실행하므로 로그인 해결 시 함께 해결됨.
  - **`google-services.json` 업데이트**: 두 PC의 SHA-1 모두 포함된 파일로 교체. (gitignore 파일이므로 각 PC에 수동 보관 필요)
  ...
  - **비로그인 시 AI 입력 차단**: 로그인 없이 ✨ AI 입력 버튼 클릭 시 AI 분석 없이 저장되던 문제 수정. 비로그인 상태에서는 설정 화면(로그인 유도)으로 연결.
  - **Claude Code 자동 승인 설정 정리**: `~/.claude/settings.json` 중복 항목 제거, `Bash(tail:*)` · `Bash(keytool:*)` 와일드카드 추가로 22개 → 17개로 간소화.
  - **CLAUDE.md 업데이트**: SHA-1 핑거프린트 관리 섹션 추가, 자동 승인 설정 전체 JSON 명시 — 다른 PC에서 복붙으로 바로 적용 가능.
