...
## 2026-04-29 — 릴리즈 노트 정합성 일괄 수정 (세션 54)

### 작업 배경
- GitHub Release `v1.1.4`부터 `v1.1.8`까지 본문이 자동 템플릿(커밋/빌드 시각)만 포함되어 실제 변경사항이 누락된 상태 확인.
- GitHub 이슈 [#144](https://github.com/jeiel85/BrioDo/issues/144) 생성 후 정비 진행.

### 수정 내용
1. **기존 릴리즈 노트 일괄 보정**
   - 대상: `v1.1.4`, `v1.1.5`, `v1.1.6`, `v1.1.7`, `v1.1.8`
   - 각 릴리즈 본문을 사용자 관점 `주요 변경` + `기술 변경` 형식으로 교체.
2. **개발 지침 보완 (`AGENTS.md`)**
   - 릴리즈 노트 필수 반영 규칙 추가.
   - 태그 릴리즈 후 `gh release view` 검증 및 `gh release edit` 즉시 수정 절차 명문화.
   - 단순 버전 범프 릴리즈도 메타데이터 변경(versionName/versionCode) 명시 의무화.

### 결과
- `v1.1.4` 이후 누락되어 있던 릴리즈 노트가 실제 변경사항 기준으로 정상 반영됨.
- 향후 동일 문제 재발 방지를 위한 운영 규칙이 문서에 고정됨.

---

## 2026-04-23 — 폰트 크기에 따른 UI 밀도 개선 (세션 53)

### 작업 배경
- 사용자가 폰트 크기를 작게 설정했을 때, 폰트만 작아지고 UI 밀도(여백 등)는 그대로 유지되어 화면 활용도가 떨어지는 문제 제기.
- GitHub 이슈 #143을 생성하여 관리.

### 수정 내용
1. **개발 지침 업데이트**: `CLAUDE.md`에 GitHub 이슈 관리 및 버전 업/릴리즈 규칙 추가.
2. **UI 밀도 조절 로직 도입**:
...
#### E2E 테스트 자동화
- **Playwright E2E 테스트**: 기존 27개 테스트 유지
  - 기본 동작 (페이지 로드, 헤더 표시)
  - 할 일 관리 (입력 버튼, 모달, 추가)
  - 설정 화면 (설정 버튼, 모달)
  - 반응형 레이아웃 (모바일, 태블릿)
- **3 라운드 전체 테스트 통과**: 81개 테스트 실행, 문제 없음

#### 시각적 회귀 테스트 (신규)
- **시각적 테스트 스위트**: 102개 테스트 신규 추가
  - 레이아웃 오버플로우 자동 감지
  - 텍스트 잘림 감지 (overflow:hidden 상태)
  - 요소 오버랩 분석
  - 7개 브레이크포인트 반응형 테스트 (320px ~ 1920px)
  - UI 상태별 스크린샷 캡처
  - 요소 위치 검증
- **테스트 브라우저**: Chromium, Android Chrome, iOS Safari
- **통과율**: 100% (129개 테스트)

### 생성된 파일
- `tests/e2e/visual.spec.js` — 시각적 회귀 테스트 스위트
- `tests/e2e/helpers/visual.js` — 레이아웃 분석 유틸리티
- `playwright.config.js` — 시각적 테스트 설정 추가

### 테스트 결과
| 구분 | 결과 |
|------|------|
| **전체 E2E 테스트** | 27개 (기능) + 102개 (시각적) = 129개 |
| **통과율** | 100% |
| **스크린샷** | `test-results/visual-screenshots/`에 자동 저장 |

### 감지된轻微 이슈 (정상 동작)
- 헤더 날짜 텍스트 잘림: ellipsis 처리되어 정상
- FAB-할 일 목록 오버랩: FAB 우하단 위치의 정상 동작
- 데스크톱 오버플로우: overflow:auto 스크롤 영역으로 정상
...
#### AI 아침/저녁 브리핑 기능 (Issue #69)
- 매일 아침/저녁 설정한 시간에 AI가 오늘 할 일 브리핑을 생성해 알림으로 전달
- 설정에서 브리핑 시간 독립 설정 가능

#### 가로(landscape) 3열 레이아웃 (Issue #41)
- landscape 방향 또는 min-width 600px 이상에서 좌측 세로 내비게이션 + 3열 분리 레이아웃
- 태블릿/폴더블 기기 UX 개선

#### 설정 화면 업데이트 상태 표시
- 설정 열릴 때 자동 업데이트 체크, 최신 여부 뱃지로 즉시 표시

#### 기본 보기 설정 버그 수정
- '목록' 탭으로 이동 후 설정을 열면 기본 보기가 미설정으로 보이던 문제 수정
- `defaultView`를 현재 탭(`viewMode`)과 분리, `briodo-defaultView`로 독립 저장
...
### 배경
- 이슈 #49: 화면 좌/우 스크롤로 탭 이동하는 UX 요청

### 변경 내용
- **좌우 스와이프 탭 전환** (`App.jsx`) — `todo-list-section`에 `onTouchStart`/`onTouchEnd` 핸들러 추가
  - 탭 순서: 오늘(date) ↔ 인박스(all) ↔ 컬렉션(lists)
  - 조건: 수평 이동 50px 이상 + 수평 > 수직 (수직 스크롤과 충돌 방지)
  - 기존 `switchTab` 함수 재사용 (검색 초기화 포함)
- #72 종료 — AdMob 이미 제거됨
...
### 배경
- Galaxy Store Seller Portal 유료 판매자 심사 통과 → 앱 등록 시작
- v1.0.2 (versionCode 4) 릴리즈 AAB 빌드 + GitHub Release v1.0.2 생성

### 변경 내용
- **헤더 컴팩트 바 완성** (이슈 #107~#115)
  - 인박스 기간 선택 버튼 컴팩트 바로 이동 (스크롤 후에도 접근 가능)
  - 오늘/인박스 탭 컴팩트 바에 태그 필터 추가
  - 컴팩트 헤더에서 브리오/검색/알림 버튼 제거
  - 탭별 정체성 인디케이터: 오늘 `완료율%` / 인박스 `잔여 배지` / 컬렉션 `진행 바` / 통계 `%`
- **헤더 상단 바 정리** — 날씨·브리오를 그리팅 영역으로 이동, 검색·알림만 유지
- **설정 전체화면 전환** — `border-radius: 0`, `height: 100vh`
- **버그 수정**: 오늘 탭 컴팩트 바 태그 필터 중복 해소

### 출시 이력
- **Samsung Galaxy Store**: v1.0.2 최초 업로드 (심사 중)
...
### 수정 내용
- **#85 온보딩 글씨 가시성**: CSS 변수 의존 제거 → 브랜드 고정 컬러 (다크 네이비 + 흰 텍스트). 테마 미적용 타이밍·삼성 One UI 등 환경 무관하게 항상 선명
- **#84 날씨 지역명 삭제**: 지역명 입력란에 ✕ 지우기 버튼 추가 (값 있을 때만 표시)
- **개인정보처리방침**: `https://jeiel85.github.io/BrioDo/privacy-policy.html` — 이미 배포 완료 확인
- **출시 블로커 현황**: Google OAuth 심사 제출 + Play Console 신원 인증 필요 (수동)
