...
| 기능 | 설명 |
|------|------|
| **스마트 입력** | 자연어 텍스트 또는 음성으로 입력 → Gemini AI가 날짜/시간/태그 자동 추출 |
| **음성 입력** | 네이티브 음성 인식(`@capacitor-community/speech-recognition`)으로 마이크 입력 지원 |
| **AI 자연어 처리** | "내일 오후 3시 팀 회의" → 날짜/시간 자동 추출, 제목 정제, 태그 자동 추천 (Gemini 2.5 Flash) |
| **Google 캘린더 연동** | BlendDo 전용 캘린더 자동 생성 및 양방향 동기화, 다중 기기 공유, 삭제 시 자동 복구 |
...
| **가변 폰트 크기** | 프리셋 3단계 버튼 + 슬라이더 1~7단계 세밀 조정 |
...
- **AI**: Google Gemini AI (`@google/genai`, gemini-2.5-flash-lite, multi-model fallback)
- **Speech**: `@capacitor-community/speech-recognition` (네이티브 Android 음성 인식)
...
| 파일 | 위치 | 설명 |
|------|------|------|
| `stitch_add_edit_task.zip` | `assets/design/` | Stitch(Google) 기반 할 일 추가/편집 화면 UI 디자인 가이드. 컴포넌트 레이아웃, 입력 모달 구조, 스마트 입력 UX 참고 자료 |
...
│   ├── InputModal.jsx             # 할 일 추가/수정 모달 (수동 입력, AI 없음)
│   ├── SmartInputModal.jsx        # 스마트 입력 모달 (음성+텍스트, AI 자동 분석)
│   └── SettingsModal.jsx          # 설정 모달 (테마, 폰트, 입력 방식, 로그인)
│
├── constants/
